...
# bart(forward里的一个if删了一个条件 past_key_value[0].shape[2] == key_value_states.shape[1])
...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if sentence_mask is None:
            ...
            alpha = 0.5
            hidden_states = alpha * (hidden_states * new_weights) + (1-alpha) * hidden_states
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        layerAttn_outputs = self.layerAttn(encoder_hidden_states, attention_mask)
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
            # encoder_hidden_states=encoder_outputs[0],
            encoder_hidden_states = layerAttn_outputs,
...
class CenBartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        use_copyer: Optional[bool] = False
    ) -> Union[Tuple, Seq2SeqLMOutput]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should either be in `[0, ...,
        ...
        if labels is not None:
            ...
            if use_copyer:
                #_decoder_input_ids = shift_tokens_right(decoder_input_ids, self.config.pad_token_id)
                #decoder_input_embeddings =  self.model.shared(shift_tokens_right(decoder_input_ids, self.config.pad_token_id, self.config.decoder_start_token_id)
                decoder_input_embeddings = self.model.shared(decoder_input_ids)
                ...
                if not isinstance(decoder_attention_mask, torch.Tensor):
                    decoder_attention_mask = (input_ids != self.config.pad_token_id).long()
                condition = decoder_attention_mask != 0
                pointer = torch.where(condition, true_tensor, false_tensor)
                #print(decoder_input_embeddings)
                ...
                masked_lm_loss = 0.7 * masked_lm_loss + 0.3 * copy_loss
                ...
                #if not isinstance(decoder_attention_mask, torch.Tensor):
                  #  decoder_attention_mask = (input_ids != self.config.pad_token_id).long()
                    #raise ValueError("decoder_attention_mask should be a PyTorch Tensor")
                #condition = decoder_attention_mask != 0
                #pointer = torch.where(condition, true_tensor, false_tensor)
                #pointer = torch.where(decoder_attention_mask != 0, true_tensor, false_tensor)
                #copy_prob = self.copyer(decoder_input_embeddings, outputs[1][-1], pointer)
                #copy_loss = copy_prob.masked_select(pointer).mean()
                #masked_lm_loss = 0.7 * masked_lm_loss + 0.3 * copy_loss
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        # bart只有input_ids、past_key_values、attention_mask、use_cache
        return {
            "input_ids": None,  # encoder_outputs is defined. input_ids not needed
            "encoder_outputs": encoder_outputs,
            "past_key_values": past,
